...
class DatasetInputOutputConsistency(unittest.TestCase):
    ...
    def get_ref_var_types(self, multitable, data_type="df", schema=None):
        ...
        if not multitable:
            ...
            #  - the type of "Date" from Timestamp to "Date" as Khiops can detect it
            # -  the type of "Recommended IND" because Khiops doesn't parse it well
            print(data_type)
            if data_type == "file":
                ref_var_types["Reviews"]["Date"] = "Date"
                ref_var_types["Reviews"]["Recommended IND"] = "Categorical"
                warnings.warn("Changed field `Recomended IND` to avoid a Khiops bug")
        elif schema == "star":
            ...
            assert schema == "snowflake", f"'schema' should be 'snowflake' no {schema}"
            ref_var_types["A"] = {
                "User_ID": "Categorical",
                "class": "Categorical",
                "B": "Table",
    ...
    def test_create_khiops_domain(self):
        ...
        for fixture in fixtures:
            with self.subTest(**fixture):
                ...
                # print(ds.to_spec())
                # if fixture["data_type"] == "file" and len(ds.secondary_tables) > 3:
                # if ds.secondary_tables:
                # print(ds.secondary_tables[3].data_source)
                # import shutil
                # shutil.copy(ds.secondary_tables[3].data_source, "./test.txt")
    ...
    def _test_domain_coherence(self, ds, ref_var_types):
        ...
        out_table_names = set(dictionary.name for dictionary in out_domain.dictionaries)
        self.assertEqual(ref_table_names, out_table_names)

        # Check that the main table is root iff for multitable
        self.assertEqual(
