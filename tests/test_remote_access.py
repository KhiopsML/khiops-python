...
def s3_config_exists():
    # The alternate env vars...
    # (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_ENDPOINT_URL`)
    # plus the bucket name in `S3_BUCKET_NAME`
    # are also usable instead of the credentials and config files
    # but are regarded unsecure
    return (
...
class KhiopsS3RemoteFileTests(KhiopsRemoteAccessTestsContainer.KhiopsRemoteAccessTests):
    ...
    def setUpClass(cls):
        ...
        if s3_config_exists():
            ...
            # whereas root_temp_dir (where the log file is saved by default)
            # can be remote
