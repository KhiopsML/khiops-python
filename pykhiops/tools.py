...
def convert_pk10_main():
    ...
    parser = argparse.ArgumentParser(
        ...
        description="converts a pykhiops 9 script to a pykhiops 10 script",
    )
    parser.add_argument(
    ...
    with open(input_path) as script_file, open(output_path, "w") as out_file:
        for line in script_file:
            if line.startswith(import_line):
                ...
                print(f"from pykhiops import core as {tokens[1]}\n", file=out_file)
    ...
    translator_dict.update(
        {
            ...
            "getPyKhiopsDir": "get_pykhiops_core_dir + str",
...
def _build_keywords_regex():
    ...
    while i < num_keywords:
        ...
        if i == current_keyword_index + 1:
            pykhiops9_keywords_regexes.append(current_keyword)
        else:
            pykhiops9_keywords_regexes.append(f"{current_keyword}[a-zA-Z]*")
        current_keyword_index = i
    return "|".join(pykhiops9_keywords_regexes)
