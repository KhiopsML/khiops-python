...
def _check_dictionary_compatibility(
    ...
    model_variable_names = set(var.name for var in model_dictionary.variables)
    ds_variable_names = set(var.name for var in ds_dictionary.variables)
    ...
    if len(model_var_names_not_in_ds) > 0:
        if target_variable_name is None:
            ...
            effective_model_var_names_not_in_ds = model_var_names_not_in_ds - set(
...
class KhiopsCoclustering(KhiopsEstimator, ClusterMixin):
    ...
    def _transform_prepare_deployment_model_for_predict(self, ds):
        # This function is a fixed pattern and in this case it doesn't need ds
...
class KhiopsPredictor(KhiopsSupervisedEstimator):
    ...
    def _transform_prepare_deployment_model_for_predict(self, ds):
        ...
        # Remove the target variabale if it is not present in the input dataset
