...
def check_table_entry(table_name, table_spec):
    ...
    if not isinstance(source, (pd.DataFrame, spmatrix, str)) and not hasattr(
        source, "__array__"
    ):
        raise TypeError(
            type_error_message(
...
def get_khiops_type(numpy_type):
    """Translates a numpy type to a Khiops dictionary dtype
...
class Dataset:
    ...
    def __init__(self, X, y=None, categorical_target=True, key=None):
        ...
        ), "'target_column' is None but y wasn't"
    ...
    def _init_tables_from_mapping(self, X):
        ...
        elif isinstance(main_table_source, spmatrix):
    ...
    def _init_target_column(self, y):
        ...
        if isinstance(self.main_table, PandasTable) and isinstance(y_checked, str):
            raise TypeError(
        ...
        # Case when y is column id: Set both the column and the id to it
            ...
            if self.target_column_id not in self.main_table.column_ids:
                raise ValueError(
                    ...
                    f"not present in columns '{self.main_table.column_ids}'"
                )
