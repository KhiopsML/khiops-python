...
from khiops.core.exceptions import PyKhiopsEnvironmentError, PyKhiopsRuntimeError
...
class PyKhiopsRunner(ABC):
    ...
    def _init_root_temp_dir(self):
        ...
        strategies in order:
        - Check that ``$TEMP/khiops`` exists and use it
        - Try to create ``$TEMP/khiops` and use it
        - Create a ``$TEMP/khiops_<HASH>`` and use it
        """

        # Create the directory if it doesn't exists
        self._root_temp_dir = os.path.join(tempfile.gettempdir(), "khiops")
        ...
        # permissions or if it is a file
            ...
            self._root_temp_dir = tempfile.mkdtemp(prefix="khiops_")

    @property
    def root_temp_dir(self):
        r"""str: The runner's temporary directory

        The temporary scenarios/templates and dictionary files created by khiops are
        stored here.
    ...
    def _build_status_message(self):
        ...
        """
        status_msg = f"Khiops Python {khiops.__version__} info and defaults\n"
        status_msg += f"Khiops Python runner class    : {self.__class__.__name__}\n"
        status_msg += f"Khiops Python max cores       : {self.max_cores}"
        if self.max_cores == 0:
            status_msg += " (no limit)"
        status_msg += "\n"
        status_msg += f"Khiops Python max memory (MB) : {self.max_memory_mb}"
        if self.max_memory_mb == 0:
            status_msg += " (no limit)"
        status_msg += "\n"
        status_msg += f"Khiops Python temp dir        : {self.root_temp_dir}"
        return status_msg
...
_khiops_runner = PyKhiopsLocalRunner()
...
def _get_tool_info_khiops10(runner, tool_name):
    ...
    *This method is deprecated and kept only for backwards compatibility*
    ...
    ------
    `.PyKhiopsEnvironmentError`
        If the current khiops runner is not of class `.PyKhiopsLocalRunner`.
    """
    # Get the version
