...
def convert_kh10_entry_point():  # pragma: no cover
    """Entry point of the convert-kh10 script"""
    ...
    convert_ok = convert_kh10(args.input_path, args.output_path)
    if convert_ok:
        sys.exit(0)
    else:
        sys.exit(1)
...
def convert_kh10(input_path, output_path):  # pragma: no cover
    """Main function for the convert-kh10 script"""
...
def _build_translator_dict():  # pragma: no cover
    ...
    translator_dict = {
        keyword: _camel_to_snake(keyword) for keyword in KHIOPS9_KEYWORDS
    }
...
def _build_keywords_regex():  # pragma: no cover
    ...
    i = 1
    num_keywords = len(KHIOPS9_KEYWORDS)
    while i < num_keywords:
        current_keyword = KHIOPS9_KEYWORDS[current_keyword_index]
        while i < num_keywords and KHIOPS9_KEYWORDS[i].startswith(current_keyword):
            i += 1
